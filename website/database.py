...
def update_course(key: str, name: str, credits: int, status: str):
    ...
    sql = text('''
                    UPDATE "Courses"
                    SET
                    name=:name, credits=:credits, open=:status
                    WHERE
                    tag=:key AND visible=true
            ''')
...
def get_users_list_sorted_by(column: str, desc: bool=False):
    ...
    elif column == "username" and not desc:
        sql = text('''
                        SELECT id, username, role
                        FROM "Users"
                        WHERE visible=true
...
def get_students_list_sorted_by(column: str, desc: bool=False):
    ...
    elif column == "credits" and not desc:
        sql = text('''
                        SELECT id, username, credits
                        FROM "Students"
                        WHERE visible=true
...
def get_courses_list_sorted_by(column: str, desc: bool=False):
    column.lower()
    sql = "-"
    if column == "name" and not desc:
        sql = text('''
                        SELECT name, tag, credits, open
                        FROM "Courses"
                        WHERE visible=true
                        ORDER BY name
                ''')
        
    elif column == "name" and desc:
        sql = text('''
                        SELECT name, tag, credits, open
                        FROM "Courses"
                        WHERE visible=true
...
def delete_user(username: str):
    ...
    if not functions.new_role_request(username):
...
def search_user_in_course(key: str, username: str):
    ...
    username.lower()
    sql = text('''
                    SELECT course_id, course_tag, user_id, username, grade
                    FROM "InCourse"
                    WHERE course_tag=:key
...
def join_course(key: str, username: str):
    ...
    sql = text('''
                    ...
                    (course_id, course_tag, user_id, username, grade)
                    VALUES
                    (:course_id, :course_tag, :user_id, :username, :grade)
            ''')
...
def get_user_courses(username: str):
    username.lower()
    sql = text('''
                    SELECT "Courses".name, "Courses".credits, "Courses".tag
...
def get_course_students(key: str):
    key.lower()
    sql = text('''
                    SELECT "InCourse".username, "InCourse".grade
                    FROM "InCourse", "Students"
                    WHERE "InCourse".course_tag=:course_tag
                    AND "InCourse".username="Students".username
            ''')
...
def get_student_completed_courses_sorted_by(username: str, column: str, desc: bool=False):
    ...
    if column == "name" and not desc:
        sql = text('''
                        SELECT "Courses".name, "Courses".credits, "Courses".tag, "InCourse".grade
                        FROM "Courses", "InCourse"
                        WHERE "Courses".tag="InCourse".course_tag
                        AND "InCourse".username=:username
    ...
    elif column == "grade" and not desc:
        sql = text('''
                        SELECT "Courses".name, "Courses".credits, "Courses".tag, "InCourse".grade
                        FROM "Courses", "InCourse"
                        WHERE "Courses".tag="InCourse".course_tag
